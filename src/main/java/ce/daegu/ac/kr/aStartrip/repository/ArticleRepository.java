...
public interface ArticleRepository extends JpaRepository<Article, Long> {
    ...
    @Query("SELECT a FROM Article a WHERE (LOWER(a.title) LIKE LOWER(CONCAT('%', :inputQuery, '%')) OR LOWER(a.member.email) LIKE LOWER(CONCAT('%', :inputQuery, '%'))) AND a.articlePermission = 'OPEN'")
    Page<Article> findByTitleContainingOrMemberEmailContaining(@Param("inputQuery") String inputQuery, Pageable pageable);
    //제목 OR 작성자로 검색 AND open 인 상태만 검색, concat은 문자열 이어붙이는 역할임. LOWER는 lowercase임.
}
