...
public interface ArticleRepository extends JpaRepository<Article, Long> {
    ...
    @Query("SELECT a FROM Article a WHERE (LOWER(a.title) LIKE LOWER(CONCAT('%', :inputQuery, '%')) OR LOWER(a.member.name) LIKE LOWER(CONCAT('%', :inputQuery, '%')) OR LOWER(a.member.email) LIKE LOWER(CONCAT('%', :inputQuery, '%'))) AND a.articlePermission = 'OPEN'")
